<p align="center">
  <strong>PrexorCloud Design System</strong><br>
  <em>The canonical Reef tokens, voice, and visual language every surface implements.</em>
</p>
...
> A serious-feeling infrastructure design system for a self-hosted, open-source
...
`tokens.json` is the source of truth. Edit it, regenerate `dist/`, and commit both:

```bash
cd design-system
node build-tokens.mjs        # tokens.json → dist/{tokens.css,tokens.ts,tokens.json}
node --test __tests__/       # parity + contrast + surface-drift guards
```

CI fails if `dist/` is stale or if any surface (`dashboard/`, `website/`,
`installer/`) drifts from the canonical scales. The full spec — palette, type,
components, voice, CLI tokens, logo — follows below.
