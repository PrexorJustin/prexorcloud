...
### H.3 i18n-Coverage (~3 d) — ⏳ **Dashboard-Seiten + Leaf-Komponenten extrahiert; ~104 Residuen in Dialogen/Chrome offen; Website-DE offen**
...
- Dashboard: alle `app/pages/**` + Leaf-Komponenten extrahiert in `i18n/en.json`, `i18n/de.json`.
  ...
  - **Gate-Status:** Vorerst **soft** in CI (`ci.yml`, Warnung — analog zu den bestehenden eslint/prettier/vitest-Soft-Gates), da die 104 Residuen noch nicht extrahiert sind. **Härtet zu einem Fail**, sobald sie raus sind. Eine Handvoll language-neutraler Fälle (Beispiel-Hostnames, CLI-Snippets, ARIA-Landmark-Namen) ist in der `ALLOW`-Liste des Linters mit Begründung dokumentiert.
  - **Nächster Schritt:** Die 104 Residuen in fokussierten Batches extrahieren (en+de, `useI18n` nachverdrahten), dann den Gate hart schalten.
