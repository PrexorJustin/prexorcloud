...
2. **Soft-Limits** (~3 d) — ⏳ **offen**: Pro-Modul-Quota in `clusterConfig.modules.<id>.resources`: `maxCpuMillisPerMinute`, `maxAllocatedMbPerMinute`, `maxOpenFiles`. Überschreitung → Warn + Metrik (`prexorcloud.module.quota.exceeded`). Liest die `ModuleResourceTracker.Snapshot` aus Stufe 1.
