...
- **C.4 Scaffolder: target-selektierte Scaffolds baubar + `--no-rest`** — `--mc-plugin X` prunt jetzt `module.yaml`-`extensions:` **und** `build.gradle.kts`-`extensionArtifacts` in Lockstep (inkl. bedrock-geyser-Drop) → `preparePlatformManifest` grün; `--no-rest` löscht `rest/` + `onRegisterRoutes`-Override. End-to-end gradle-verifiziert. Go-Tests `TestPruneExtensionsBlock`/`TestPruneExtensionArtifacts`/`TestStripOnRegisterRoutes` u.a.
...
- **`--mc-plugin`-Scaffolds sind jetzt baubar.** Der `extensions:`-Block in `module.yaml` **und** die `extensionArtifacts`-Map in `build.gradle.kts` werden in **Lockstep** auf die gewählten Plattformen geprunt (`filterManifestExtensions`/`pruneExtensionsBlock` + `filterBuildGradleArtifacts`/`pruneExtensionArtifacts`, Match über `target:`-Pfadsegment bzw. `:plugin:<target>`). Das schließt zwei vorher latente Defekte: (1) Manifest und Build-Deklaration mussten exakt übereinstimmen, sonst failt `preparePlatformManifest` („declared … not referenced"); (2) der `bedrock-geyser`-Eintrag (für den das Template **kein** `plugin/`-Subprojekt mitliefert) wird mitgeprunt — genau das machte target-selektierte Scaffolds bisher nicht baubar. End-to-end verifiziert: ein `paper,folia,velocity`-Scaffold baut die Manifest-Stufe jetzt grün. Ohne Selektion bleibt die volle Liste.
...
**Bewusst out of scope (eigenes follow-up):** `--no-mongo`-Strip-Out — die `repository`→`queryService`→`capabilityHandles`→`healthCheck`-Kette des `example`-Moduls ist zu verzahnt für sichere String-Surgery; braucht eine Template-Restrukturierung. **Bekannter Rest-Defekt:** ein *plain* Scaffold (ohne `--mc-plugin`) behält den `bedrock-geyser`-Eintrag und ist daher weiter nicht baubar (Template-Issue), bis das `example` entweder ein `plugin/bedrock-geyser`-Subprojekt bekommt oder den Eintrag verliert.
