...
- ◐ **Inc 4 (partial): CLI done** (`f5250fa`). The `prexorctl deploy` tree already had `deploy`/`list`/`show`/`pause`/`resume`/`rollback`; updated for the new reality — `rollback` help no longer claims state restore is operator-driven (it reverts now), added `--min-healthy-tps`, clarified `--strategy` (rolling/canary), and `deploy show` surfaces `minHealthyTps` + `replacementTimeoutSeconds`. **Remaining:** Dashboard deployment actions (`workloads/deployments.vue` is a read-only timeline — rollback/pause/resume buttons are net-new frontend) and **deploy-back** ("save running instance → new template version" — a substantial new backend feature touching the daemon file-snapshot + template versioning).
