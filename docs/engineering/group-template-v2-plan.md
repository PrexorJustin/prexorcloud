...
- ✅ **Inc 2: rollback actually reverts** (`c26de19`). Every deployment now snapshots the full `GroupConfig` (`DeploymentRecord.groupSnapshot`, persisted; `MongoGroupStore.toJson`/`fromJson`). A rollback — manual `…/rollback` endpoint **and** the reconciler's auto-rollback (via an injected `RollbackAction`) — restores the most recent COMPLETED snapshot (`GroupManager.update` + `groupStore.save` + `GroupUpdatedEvent`) and re-deploys it as a fresh deployment linked via `rollbackOf`. New `DeploymentRollbackService`; a rollback deployment never auto-rolls-back again (loop guard on `trigger=="rollback"`). `deploymentToJson` deliberately omits the snapshot (no API leak; openapi regenerated). Tests: `DeploymentRollbackServiceTest` + reconciler auto-rollback-trigger/loop-guard tests. **NOT live-tested** (fleet jar predates it).
- ✅ **Inc 3: CANARY strategy + TPS gate** (`dbe68bc`). `updateStrategy=CANARY` resolves safe defaults in `resolveTriggerOptions` (1 canary instance, healthGate + autoRollback on, 30s stabilisation, TPS floor 18.0 = the scaler's `TPS_DEGRADED_FLOOR`), each overridable per request; a CANARY group's restart is canary-gated too (`GroupRoutes` now routes through `resolveTriggerOptions`). The reconciler health gate gains `countTpsRegressedUpdatedInstances`: an updated instance stabilised past `minHealthySeconds` but reporting `tps1m` in (0, `minHealthyTps`) fails the wave → rollback when auto-rollback is on; 0-TPS (no data / not a game server) ignored. New `DeploymentRolloutConfig.minHealthyTps`. Tests for CANARY defaults + the TPS gate. **NOT live-tested.**
- ✅ **Inc 4 CLI** (`f5250fa`). The `prexorctl deploy` tree already had `deploy`/`list`/`show`/`pause`/`resume`/`rollback`; updated for the new reality — `rollback` help no longer claims state restore is operator-driven (it reverts now), added `--min-healthy-tps`, clarified `--strategy` (rolling/canary), and `deploy show` surfaces `minHealthyTps` + `replacementTimeoutSeconds`.
- ✅ **Inc 4 Dashboard actions** (`8a123fa`). The cross-group deployments timeline (`workloads/deployments.vue`) gains per-row **pause** (in-progress), **resume** (paused), and **rollback** (completed/failed, confirm dialog) wired to the controller endpoints via the aggregate store's new `runAction`; a pure `availableActions(state)` drives per-state visibility (unit-tested). en/de locales added in parity; vitest 12/12, locale-parity OK, my files typecheck+eslint clean (dashboard typecheck has a wide pre-existing `'unknown'` baseline). NOT visually verified (no UI render here).
- ✅ **deploy-back (config-only MVP)** (`fe52f66`). `DeployBackService` walks a RUNNING instance's files via the `InstanceFileAccess` capability, captures the text/config files (binary + oversize skipped and reported), writes them into the target template's files dir, and rehashes → a new template version future instances inherit. `POST /api/v1/services/{id}/save-to-template` (TEMPLATES_UPDATE) + `prexorctl deploy save <instance> --to-template <name>`. **Config-only by design** — world/binary data round-trips lossily over the UTF-8 file RPC, so it needs a future daemon tar capability (`prexor.instance.snapshot`, noted in `InstanceFileAccess` javadoc). Tests: `DeployBackServiceTest`.

**Phase 5 (Rollout v2) is now functionally COMPLETE** — footgun fix, real rollback, CANARY + TPS gate, CLI, Dashboard actions, deploy-back. All controller/CLI/dashboard code, unit-tested, **NONE live-validated** (the deployed fleet jar predates the entire effort). World-data deploy-back is the only deliberate follow-on.
