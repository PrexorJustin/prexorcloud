...
- ◐ **Warm pool** (operator wants it). Design: a `boolean warm` flag on the instance (RUNNING but held back from routing), **not** a new `InstanceState` — a flag touches only the routing filter + DTO plumbing, while a new state would cascade through the FSM, validators, and UI. **Foundation done + committed:** `GroupConfig.warmPoolMinPrepared`, `InstanceInfo.warm` + `withWarm`, `InstanceRegistry.updateWarm`, and `ClusterState.servingInstances`/`warmInstanceCount`/`promoteWarmInstance`/`markInstanceWarm` (tested). `planHash` unchanged (golden test green). **Remaining, two focused chunks:**
  1. **Scheduler engine** — extend `GroupPlan` with a promote-count + warm-refill-count; make `SchedulerDesiredStatePlanner.planGroup` promote a warm instance before cold-starting and keep the pool refilled, and have the Scheduler executor act on it.
  2. **Proxy routing exclusion (Increment 2)** — thread `warm` through `InstanceDtoMapper` → `InstanceDto` → SSE → proxy `InstanceView`, and filter `RUNNING && !warm` at `VelocityCloudClient` + the BungeeCord equivalent + `NetworkRouter`. Until this lands, warm instances are still joinable (a soft pool).
