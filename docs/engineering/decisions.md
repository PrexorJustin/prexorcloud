...
- If we need to point operators at trends, we can add an alert-rules `.yml` reference in `observability.md`. That is more useful than a dashboard.

**Trade-off.** Operators who *would* have used a turnkey pack do extra work. We accept that — the metrics are stable and well-named, and PromQL examples in [`observability.md`](observability.md) cover the common queries.
...
- The "cross-tab UX" question in `refactor-backlog.md` is a non-problem; resolved by being explicit that the CLI context is CLI-local.
...
1. The new manifest opts in — `backend.controller.reloadable: true` (an additive optional field on manifestVersion 2; see ADR for the v2 bump under MASTER_PLAN 4.3).
