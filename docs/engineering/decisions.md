...
**Phase 3 update (config rules — landed, 2026-06-26).** The per-platform config patcher is now data-driven and the daemon holds no platform knowledge. Platform config edits live as `ConfigRule`s (`file` + selector `path` + `op` ∈ {SET, REPLACE, REGEX} + `value`): the controller's `PlatformConfigDefaults` table supplies the `base-<platform>` slice, `ConfigRuleResolver` collapses it against the group's `configPatches`, and the daemon's `ServerConfigPatcher` applies them with a single platform-agnostic engine (the per-platform `patchPaper/Spigot/Bungeecord/Geyser` methods are gone; Paper's cache-time velocity baking moved into `PaperBootstrapCache`). Per-instance scalars (port/max-players/MOTD) ride the shipped files' `%VAR%` placeholders + the daemon's variable-substitution stage rather than patches — MOTD now joins them as a `%MOTD%` resolved variable. Two deliberate seam moves, both flagged here as intended: (1) the `daemon_service.proto` `ConfigPatch` message gains an **additive** `op` field (field 4) + a `ConfigPatchOp` enum — `PROTOCOL_VERSION` stays 1; (2) because a plain Paper group's resolved patch set changed (only the velocity-forwarding secret remains a rule), the frozen `planHash` golden value was **re-pinned** and a fleet-wide re-roll is expected. Increment 1 (the `ConfigRule` model + resolver) shipped as `e2ac967`.
