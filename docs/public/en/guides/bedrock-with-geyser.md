...
```mermaid
flowchart LR
  BC["Bedrock client<br/><sub>Minecraft Bedrock Edition</sub>"]
  subgraph Gate["bedrock-gate · GEYSER Instance"]
    G["Geyser<br/><sub>translate + forward</sub>"]
  end
  subgraph Edge["edge · Velocity proxy Group"]
    R["NetworkRouter<br/><sub>detect(uuid) → bedrock</sub>"]
  end
  LB["lobby-bedrock<br/><sub>bedrockLobbyGroup</sub>"]
  SB["survival-bedrock<br/><sub>bedrockFallbackGroups</sub>"]

  BC -- "Bedrock %PORT%" --> G
  G -- "remote.address/port<br/>(injected at provision)" --> R
  R -- "join → bedrockLobbyGroup" --> LB
  R -. "kick → fallback" .-> SB
```
