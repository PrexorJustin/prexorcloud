...
description: A research-backed, diplomatic comparison of PrexorCloud and SimpleCloud V2 — features, architecture, where each one fits, and how to migrate.
---
...
| **Server platforms** | Paper, Spigot, Folia | Paper ([source](https://docs.simplecloud.app/manual/overview/)) | PrexorCloud also covers Folia. |
| **Proxy platforms** | Velocity, BungeeCord | Velocity, Bungee ([source](https://docs.simplecloud.app/manual/overview/)) | Equivalent. |
...
| **REST API** | Hand-curated OpenAPI 3.1, 151 endpoints | Available ([source](https://docs.simplecloud.app/api)) | Both expose REST; PrexorCloud's surface is larger. |
...
| **Real-time event stream** | Server-Sent Events, 22 typed events, replay via `Last-Event-ID` | gRPC streaming for typed events ([source](https://docs.simplecloud.app/api)) | Different transport for similar use case. |
| **Operator auth** | JWT + bcrypt, optional email password reset, 28 RBAC permissions | Authentication mechanisms documented ([source](https://docs.simplecloud.app/manual/overview/)) | Both authenticate operators; PrexorCloud documents RBAC count + lockout policy. |
...
| **Bedrock support** | No | No (Paper / Velocity / Bungee documented) | Out of scope for both. |
