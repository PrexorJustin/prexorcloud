...
more than the work. The reasoning behind that split is recorded in
[ADR 29 (embedded Ratis control plane)](https://github.com/prexorjustin/prexorcloud/blob/main/docs/engineering/decisions.md).
...
commit order on each peer.

```mermaid
flowchart TB
  W["Any Controller<br/><sub>grantLease · proposeConfigPatch · addMember</sub>"]

  subgraph Leader["Controller A · Raft leader"]
    LSM[("State machine")]
  end
  subgraph FollowerB["Controller B · follower"]
    BSM[("State machine<br/><sub>replica</sub>")]
  end
  subgraph FollowerC["Controller C · follower"]
    CSM[("State machine<br/><sub>replica</sub>")]
  end

  W -- "submit ClusterEntry" --> Leader
  Leader -- "append + replicate log" --> FollowerB
  Leader -- "append + replicate log" --> FollowerC
  Leader -- "apply in commit order" --> LSM
  FollowerB -- apply --> BSM
  FollowerC -- apply --> CSM
```

`ClusterControlPlane` exposes typed methods that wrap the
...
- [ADR 29 — embedded Ratis control plane](https://github.com/prexorjustin/prexorcloud/blob/main/docs/engineering/decisions.md) — why Raft over Mongo-CAS, and the coarse-vs-fine lease policy.
- [v1.0 → v1.1 upgrade runbook](https://github.com/prexorjustin/prexorcloud/blob/main/docs/runbooks/upgrade-v1.0-to-v1.1.md) — the operator path to a multi-controller quorum.
- [Cluster recovery runbook](https://github.com/prexorjustin/prexorcloud/blob/main/docs/runbooks/recover-cluster.md) — majority-loss recovery and the single-survivor reset.
