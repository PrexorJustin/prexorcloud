...
1. **The trust root does not reload live.** The verifier is instantiated
   once in `PrexorCloudBootstrap` and handed to the module manager — there
   is no cluster-config subscriber for `modules.signing` (unlike CORS /
   rate-limit / JWT, which *do* hot-reload). **A trust-root change takes
   effect only after a controller restart.**
2. **Already-installed modules are not re-verified.** Verification happens
   ...
   fresh controller that re-loads stored modules will verify against the
   new root. This is why step 3 below (re-sign + re-upload) is mandatory,
   not optional.
...
1. **Append the new key/cert to the trust-root PEM.** Concatenate the new
   **PUBLIC KEY** (or **CERTIFICATE**) block onto the existing bundle so
   ...
   just the path. Distribute the updated PEM to every controller.)
...
> **Order matters.** A module still signed only with the old key will
> **fail-closed at install** after step 4. Do not skip step 3.
