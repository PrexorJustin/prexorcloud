...
[`../architecture.md`](../architecture.md) §"HA model"). Failover is automatic; the peer picks up
the dead controller's leases within `lease.timeoutSeconds` (default
15s).
...
If a deploy / drain / rolling restart hangs after a controller restart:

```bash
# List in-flight workflows.
prexorctl workflow list --status RUNNING

# Inspect.
prexorctl workflow info <workflow-id>

# Cancel if needed (cancellation is intent-based; the reconciler
# unwinds it).
prexorctl workflow cancel <workflow-id>
```
