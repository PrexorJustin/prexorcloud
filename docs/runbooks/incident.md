...
What an on-call operator does between "page" and "post-mortem." This
runbook makes first response **fast and boring**: known signals, known
steps, known escalation path.
...
| `checks.mongo=false` in `/system/ready` for >2 min                | SEV-1    | Same                                         |
| `checks.redis=false` in `/system/ready` for >5 min                | SEV-2    | Same                                         |
...
| `prexorcloud_module_classloader_leaked_total` rate increases      | SEV-3    | Metrics                                       |
| Daemon `OFFLINE` for >5 min on >2 nodes                           | SEV-2    | `prexorctl node list` watcher                  |
| MC Instance crash rate >threshold per Group                       | SEV-2/3  | `prexorctl crash list`; `crashes.crashLoopThreshold` triggered |
...
3. **Capture state before changing it.** From any Controller host:
   ...
   prexorctl crash list --since "$(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%SZ)" > /tmp/inc-<id>-crashes.txt || true
   ```
   For SEV-1/2, take heap and thread dumps too — see
   [`recover-controller.md`](recover-controller.md) §Step 1.
4. **Identify the layer.** Use the `/system/ready` body and the symptom
...
5. **Stabilize, don't fix.** First response stops the bleeding. Resist
   the urge to root-cause before stability returns.
