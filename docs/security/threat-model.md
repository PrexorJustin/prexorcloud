...
Controllers run active-active over shared MongoDB. As of v1.1, coarse-grained mutating work (scheduler, deployment reconciler, audit pruner, DR drill) is gated by **Raft leases** on the embedded control plane; fine-grained partitioned work (per-instance placement, per-node drains) stays on Valkey leases. See the [architecture overview](../public/en/internals/architecture.md).
...
The PrexorCloud surface visible to players is indirect: the Player Journey Bus records typed transitions (CONNECTED / TRANSFER / DISCONNECTED / INSTANCE_CRASHED). Per-player metadata is covered under asset 13.
...
| Gap | Notes |
|---|---|
| Cgroup / container isolation for MC instances | Out of scope for v1. See [ADR 7 — no Kubernetes-style isolation](../engineering/decisions.md). |
| Tamper-evident audit log | Append-only by convention; cryptographic chaining is a v2 conversation. |
| Trace-context propagation across boundaries | Opt-in OpenTelemetry tracing ships as of v1.2 — off by default, a no-op when disabled. W3C `traceparent` rides the existing authenticated REST/gRPC channels only; no new external surface. See [ADR 30 — OpenTelemetry tracing opt-in](../engineering/decisions.md). |
| OIDC / SSO / MFA | Not in scope. See [ADR 8 — username + password + JWT only](../engineering/decisions.md). |
