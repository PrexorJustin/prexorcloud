...
const allModes: Mode[] = [
  {
    ...
    desc: 'Single host runs both. Best for evaluation and home labs.',
  },
  {
    id: 'controller',
    tag: 'Control plane',
    name: 'Controller only',
    desc: 'Daemons join later from other hosts. Recommended for production.',
  },
  {
    id: 'controller-join',
    tag: 'Join cluster',
    name: 'Add controller',
    desc: "Join this host to an existing controller cluster. Needs a join token from 'prexorctl cluster join-token create'.",
  },
  {
    id: 'daemon',
    tag: 'Join cluster',
    name: 'Add daemon',
    desc: 'Bring a node online for an existing controller. Needs a join token.',
  },
  {
    id: 'dashboard',
    tag: 'Web UI',
    name: 'Dashboard',
    desc: 'Standalone frontend that talks to a remote controller over HTTPS.',
  },
  {
    id: 'cli',
    tag: 'Sign in',
    name: 'CLI login',
    desc: 'Bind this prexorctl to an existing cluster. No install — just point it at the controller and sign in.',
  },
];
  ...
  <div class="body">
    <div class="content-pane">
      <div class="welcome">
        ...
        <h1>
          How do you want to
          <em class="accent-serif">run it?</em>
        </h1>
        <p>
          This decides which configuration we generate. You can change it any time — controllers and
          daemons can be added to a cluster in either order.
        </p>
      </div>
      ...
      <div class="continue-bar">
        ...
        <div class="copy">
          <template v-if="wiz.mode === 'cli'">
            You'll just enter a controller URL + credentials — no install, no files touched on this
            host.
          </template>
          <template v-else>
            You'll spend ~3 minutes here. We'll generate
            <code>{{ files }}</code>
            at the end.
          </template>
        </div>
