    ...
    <div class="content-pane">
      <template v-if="wiz.cliLoginDone">
        <div class="welcome" style="max-width:560px;margin:0 auto;text-align:center;">
          ...
          <h1 style="margin-top:8px;">You're <em class="accent-serif">connected.</em></h1>
          <p>
            prexorctl is now logged in to <code>{{ wiz.cliLoginController }}</code> as
            <code>{{ wiz.cliLoginAs }}</code>. The token has been written to
          ...
          <div style="margin-top:18px;display:flex;justify-content:center;gap:8px;">
            <button class="btn primary" type="button" @click="wiz.exitWizard()">Close wizard</button>
          </div>
        </div>
      </template>
      ...
      <template v-else>
        <div class="welcome" style="max-width:560px;margin:0 auto;">
          ...
          <h1>Sign in to your <em class="accent-serif">cluster.</em></h1>
          <p>
            Bind this prexorctl to an existing controller. We'll POST your credentials to its
            <code>/api/v1/auth/login</code>, store the returned token in
            <code>~/.prexorcloud/config.yml</code>, and you're done — no install, no systemd,
            nothing written outside that file.
          </p>
        </div>

        <div style="max-width:560px;margin:24px auto 0;display:flex;flex-direction:column;gap:14px;">
          <div>
        ...
        <div class="navbar" style="margin-top:28px;">
          ...
          >{{ wiz.cliLoginBusy ? 'Signing in…' : 'Sign in' }}</button>
        </div>
      </template>
    </div>
  </div>
